    ...
    complie(list){
        ...
        list = list.map(function(data){
            return {
                ...
                goodsImage: data.goods_original,
                goodsTitle: data.goods_title,
                shop_price: data.shop_price,
                market_price: data.market_price,
                off_enable: data.show_discount_icon == 1,
                off: data.discount,
                goods_sn: data.goods_sn,
                cat_id: data.cat_id,
                goods_id: data.goods_id,
                ratio: `${Number(data.goods_img_h)/Number(data.goods_img_w)*100}%`
            }
        });
        return list.map (data => new GoodsItem(data));
    }
    mounted(elems){

...
- 工作3年以上的老人（这部分人大多都业务十分娴熟，一个当好几个用，但是，基础比较薄弱，可能没有尝试<font color='red'>写过框架、组件、脚手架</font>等）
...
[http://fex.baidu.com/blog/2014/05/what-happen/](https://link.segmentfault.com/?enc=L7CaOIeBrRVUmC7yA%2Fw3hA%3D%3D.vphLSBxzi%2FHwEtYvC7CO%2BBweaLSChXu7OTImM1Qy68YHNIRYSBW8RJkSCaNZZwBu)
...
- 核心知识，必须掌握的，也是最基础的，譬如<font color='red'>浏览器模型，渲染原理，JS解析过程，JS运行机制等，作为骨架来承载知识体系</font>
...
知识体系中，最重要的是<font color='red'>骨架，脉络</font>。有了骨架后，才方便填充细节。所以，先梳理下主干流程：
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662131)
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662132)
...
`HTTP 1.0`定义参考：[https://tools.ietf.org/html/rfc1945](https://link.segmentfault.com/?enc=Ud0BWmFSy%2B7lP247X51kmA%3D%3D.Mhddrr8xIpD%2Fmg4UmxNrgU6pZ9yuO9VI8m0J4bX%2BTWXy18F25CXVeEkWsbR9qPe7)

`HTTP 1.1`定义参考：[https://tools.ietf.org/html/rfc2616](https://link.segmentfault.com/?enc=rLtJ%2FVr7d5ndGycMse3%2BgQ%3D%3D.otuLq%2Bmlyh0j1m7UvCzA%2FmFEegAZn81U7MxyX9Te0MlbZv01IgleuRq8FTFD3sSZ)
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662133)
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662134)

### cookie以及优化

cookie是浏览器的一种本地存储方式，一般用来<font color='red'>帮助客户端和服务端通信</font>的，常用来进行身份校验，结合服务端的session使用。
...
说到了<font color='red'>多域名拆分</font>，这里再提一个问题，那就是：
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662135)
...
- http1.1中，<font color='red'>每请求一个资源，都是需要开启一个tcp/ip连接的</font>，所以对应的结果是，每一个资源对应一个tcp/ip请求，由于tcp/ip本身有并发数限制，所以当资源一多，速度就显著慢下来
- http2.0中，<font color='red'>一个tcp/ip请求可以请求多个资源</font>，也就是说，只要一次tcp/ip请求，就可以请求若干个资源，分割成更小的帧请求，速度明显提升。
...
一般来说，如果要将网站升级成https，需要后端支持（后端需要申请证书等），然后https的开销也比http要大（因为需要额外建立安全链接以及加密等），所以

一般来说http2.0配合https的体验更佳（因为http2.0更快了）
...
这里放一张图（来源：[阮一峰-图解SSL/TLS协议](https://link.segmentfault.com/?enc=zSWkGrGlafcPLyjvtuwN5A%3D%3D.iJmB7MbSnf0raKW2TZWAIkpHE90wszUwJiSu40YtmB3d6ghJmxV7TGW8K3kYiHdxE4d3Zah3pIGY%2BBfKwJCSVA%3D%3D)）

![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662136)
...
但是对于强缓存，在未过期时，必须<font color='red'>更新资源路径</font>才能发起新的请求（<font color='red'>更改了路径</font>相当于是另一个资源了，这也是前端工程化中常用到的技巧）
...
- `Expires`：<font color='red'>服务端配置的，属于强缓存</font>，用来控制在规定的时间之前，浏览器不会发出请求，而是直接使用本地缓存，注意，Expires一般对应服务器端时间，如`Expires：Fri, 30 Oct 1998 14:19:41`
- `If-Modified-Since/Last-Modified`：这两个是<font color='red'>成对出现的，属于协商缓存</font>的内容，其中浏览器的头部是`If-Modified-Since`，而服务端的是`Last-Modified`，它的作用是，在发起请求时，如果`If-Modified-Since`和`Last-Modified`匹配，那么代表服务器资源并未改变，因此服务端不会返回资源实体，而是只返回头部，通知浏览器可以使用本地缓存。`Last-Modified`，顾名思义，指的是文件最后的修改时间，而且只能精确到`1s`以内

**http1.1中的缓存控制：**

- `Cache-Control`：缓存控制头部，有no-cache、max-age等多种取值
- `Max-Age`：<font color='red'>服务端配置的，用来控制强缓存</font>，在规定的时间之内，浏览器无需发出请求，直接使用本地缓存，注意，Max-Age是Cache-Control头部的值，不是独立的头部，譬如`Cache-Control: max-age=3600`，而且它值得是绝对时间，由浏览器自己计算
- `If-None-Match/E-tag`：这两个是<font color='red'>成对出现的，属于协商缓存</font>的内容，其中浏览器的头部是`If-None-Match`，而服务端的是`E-tag`，同样，发出请求后，如果`If-None-Match`和`E-tag`匹配，则代表内容未变，通知浏览器使用本地缓存，和Last-Modified不同，<font color='red'>E-tag更精确</font>，它是类似于指纹一样的东西，基于`FileEtag INode Mtime Size`生成，也就是说，只要文件变，指纹就会变，而且没有1s精确度的限制。
...
因此<font color='red'>推荐</font>使用`Max-Age`。
...
- 是一种指纹机制，代表文件相关<font color='red'>指纹</font>
- 只有文件变才会变，也只要文件变就会变，
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662137)
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662138)
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662139)
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662140)
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662141)
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662142)

### 渲染

有了render树，接下来就是开始渲染，基本流程如下：

![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662143)
...
<font color='red'>回流的成本开销要高于重绘</font>，而且一个节点的回流往往会导致子节点以及同级节点的回流，
所以优化方案中一般都包括，尽量避免回流。
...
- <font color='red'>减少逐项更改样式</font>，最好一次性更改style，或者将样式定义为class并一次性更新
- <font color='red'>避免循环操作dom，创建一个documentFragment或div，</font>在它上面应用所有DOM操作，最后再把它添加到window.document
- <font color='red'>避免多次读取offset等属性</font>。无法避免则将它们缓存到变量
- 将<font color='red'>复杂的元素绝对定位或固定定位</font>，使得它脱离文档流，否则回流代价会很高
...
上述中的渲染中止步于绘制，但实际上绘制这一步也没有这么简单，它可以结合<font color='red'>复合层和简单层</font>的概念来讲。
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662144)
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662145)
...
当遇到上述的外链时，会单独开启一个<font color='red'>下载线程</font>去下载资源（http1.1中是每一个资源的下载都要开启一个http请求，对应一个tcp/ip链接）
...
- 但是会<font color='red'>阻塞渲染</font>，也就是在构建render时，会等到css下载解析完毕后才进行（这点与浏览器优化有关，防止css规则不断改变，避免了重复的构建）
...
- <font color='red'>阻塞浏览器的解析</font>，也就是说发现一个外链脚本时，需等待脚本下载完成并执行后才会继续解析HTML
...
- `async`是异步执行，异步下载完毕后就会执行，<font color='red'>不确保执行顺序</font>，一定在`onload`前，但不确定在`DOMContentLoaded`事件的前或后
- `defer`是延迟执行，在浏览器看起来的效果像是将脚本放在了`body`后面一样（虽然按规范应该是在`DOMContentLoaded`事件前，但实际上不同浏览器的优化效果不一样，也有可能在它后面）

**遇到img图片类资源**

遇到图片等资源时，直接就是异步下载，<font color='red'>不会阻塞解析</font>，下载完毕后直接用图片替换原有src的地方

### loaded和domcontentloaded

简单的对比：

- DOMContentLoaded 事件触发时，仅当DOM加载完成，<font color='red'>不包括样式表，图片(譬如如果有async加载的脚本就不一定完成)</font>
- load 事件触发时，页面上所有的<font color='red'>DOM，样式表，脚本，图片都已经加载完成了</font>
...
一个元素的box的<font color='red'>定位和尺寸</font>，会与某一矩形框有关，这个框就称之为包含块。

元素会为它的子孙元素创建包含块，但是，并不是说元素的包含块就是它的父元素，元素的包含块与它的祖先元素的样式等有关系

譬如：

- 根元素是最顶端的元素，它没有父节点，它的包含块就是<font color='red'>初始包含块</font>
- static和relative的包含块由它<font color='red'>最近的块级、单元格或者行内块祖先元素的内容框（content）创建</font>
- fixed的包含块是当前可视窗口
...
- 如果一个框里，有一个块级元素，那么这个框里的内容都会被当作块框来进行格式化，因为只要出现了块级元素，就会将里面的<font color='red'>内容分块几块，每一块独占一行</font>（出现行内可以用匿名块框解决）
- 如果一个框里，没有任何块级元素，那么这个框里的内容会被当成行内框来格式化，因为里面的内容是<font color='red'>按照顺序成行的排列</font>
...
但总的来说，它就是<font color='red'>行内元素自身如何显示以及在框内如何摆放的渲染规则</font>，这样描述应该更容易理解
...
[http://bbs.csdn.net/topics/340204423](https://link.segmentfault.com/?enc=sLvSTbCGqTAKnJ%2FwwZOwwg%3D%3D.EE5AMu4eCjXwwXEsJEVAeVEhfq1JcjrhB%2B1VD39MMPWHOmleDkRQFTwBNb8eciPK)
...
原因就是JS解释器有一个[Semicolon Insertion](https://link.segmentfault.com/?enc=gCKS87FC%2BZ4Ta14HNZjYHw%3D%3D.FVAlLp79vv%2FAX2gDP1XSsENiqLPx5EiYsC4cH0%2FGd8PybGQPAI4Drsb7J5EdDZFUDiK%2F2g9knQ4AiqBYtOtR0w%3D%3D)规则，它会按照一定规则，在适当的位置补充分号
...
此阶段的内容中的图片来源：[深入理解JavaScript系列（10）：JavaScript核心（晋级高手必读篇）](https://link.segmentfault.com/?enc=KpP7yaoLCUN8HLDIBc7dUA%3D%3D.zc6dhumNmS8gzSvXfMg4b5rhpoalFQy1UFu9ormwusapBjrf%2F4SaJ%2FWNeeBJ9%2BY2LqS0DXVJDZWD%2B6mB7daNWA%3D%3D)
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662146)
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662147)
...
![img](C:\Users\guoge\Desktop\MyProject\note-gg\work\images\1460000013662148)
...
[深入理解JavaScript系列（13）：This? Yes,this!](https://link.segmentfault.com/?enc=RVtAXllFTwVGFd6NRgaqLA%3D%3D.e%2BnByuvnvTBHlbhu7hzzcVpclyGHDOKVMX7Ch1MIYZcq%2FZqp0s640Wp53pyK09exbky3v09iPZlDwyv8OJz5fw%3D%3D)
...
[V8 内存浅析](https://link.segmentfault.com/?enc=dSnAhyASM6h5o1Bxre3ppA%3D%3D.FXYC5sGw7XC1tZ7a%2Bg4MtnoD%2Fz8%2BK0a7s%2FPSr8Ts3YJ3fMnHInGnFU43AkdnADvq)
...
![img](https://sponsor.segmentfault.com/lg.php?bannerid=2&campaignid=1&zoneid=3&loc=https%3A%2F%2Fsegmentfault.com%2Fa%2F1190000013662126&referer=https%3A%2F%2Fsegmentfault.com%2Fa%2F1190000016990089&cb=a6d4a4384d)
